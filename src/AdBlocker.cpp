    ...
    : QWebEngineUrlRequestInterceptor(parent), persistent_(persistent)
{
    static const QStringList initialDomains = {
        ...
        "propellerads.com", "a-ads.com", "hilltopads.com", "clickadu.com",
        "monetag.com", "adstriker.com", "adcash.com", "adtrue.com",
        "exponential.com", "yieldmo.com", "spotxchange.com", "teads.tv",
        "admanmedia.com", "bebi.com", "clksite.com", "coinhive.com",
        "luckyorange.com", "crazyegg.com", "mc.yandex.ru", "clarity.ms",
        "doubleverify.com",
        "integralads.com", "adblade.com", "adcolony.com", "adriver.ru",
        "airpush.com", "applovin.com", "chartbeat.com", "clicktale.com",
        "conversantmedia.com", "eplanning.net", "flashtalking.com", "gumgum.com",
        "indexww.com", "kargo.com", "liveramp.com", "mathtag.com",
        "plista.com",
        "sail-thru.com", "sharethis.com", "sizmek.com", "sovrn.com",
        "tapad.com", "unruly.co", "vungle.com", "trafficjunky.net",
        "tsyndicate.com", "adxpansion.com", "ero-advertising.com",
        "ad-maven.com", "ad-maven.net", "trafficstars.com", "adskeeper.com"
    };

    for (const auto &domain : initialDomains) familyHosts_.insert(hostKey(domain));
    ...
    // Private windows inherit preferences, but never write exceptions/history to disk.
                           ...
                           QWebEngineUrlRequestInfo::ResourceType type) const
{
    ...
    if (!isEnabledForUrl(firstParty) || !request.isValid()) return false;
    if (request.scheme() != "http" && request.scheme() != "https" &&
        request.scheme() != "ws" && request.scheme() != "wss") return false;
    // A blocker must not trap explicit page visits or downloads behind a blank page.
    if (type == Info::ResourceTypeMainFrame ||
        type == Info::ResourceTypeNavigationPreloadMainFrame) return false;
    if (isVerification(request)) return false;
    if (isDomainBlocked(request.host())) return true;
    // Host-scoped endpoint rules. Query strings alone never trigger a block.
...
{
    if (!isEnabledForUrl(opener)) return false;
    return !userInitiated ||
        shouldBlock(request, opener, QWebEngineUrlRequestInfo::ResourceTypeSubFrame);
}
...
{
    QReadLocker guard(&lock_);
    return exactHosts_.size() + familyHosts_.size();
}

QString AdBlocker::filterStatus() const
{
    return QString("AdAway + LiteWave: %1 rules | %2").arg(ruleCount()).arg(
        updatedAt_.isValid() ? updatedAt_.toLocalTime().toString("yyyy-MM-dd HH:mm")
                            : "bundled 2026-09-14");
}
